...
public class DataArray {
    ...
    public static int totalLines(String fileName) {
        ...
        try (BufferedReader bf = new BufferedReader(new FileReader(new File(fileName)))){
            while(line != null) {
                line = bf.readLine();//read each line in file
                if (line != null) {
                    lineCount++;//increment line counter
                }
                else {
                    continue;//last line contingency
                }
            }
            bf.close();
        }
    ...
    public static void createArray() {
        ...
        try (BufferedReader bf = new BufferedReader(new FileReader(new File(file)))){
            while(line != null) {
                line = bf.readLine();
                if (line != null) {
                    dataArray[index] = line;//add each line to array
                }
                else {
                    continue;//null line contigency
                }
                index++;
            }
            bf.close();
    ...
    public static void randomise(int seed) {
        ...
        int randIndex0 = 0, randIndex1 = 0;//stores pseudo-randomly generated indices
        int count = seed;
        String line;
        String tempData = "";//stores the array item at some random index 
            ...
            if (seed != 0) {
                do{
                    randIndex0 = rand.nextInt(upperbound);//generate first random index
                    ...
                    if (randIndex1 != randIndex0) {
                        if (dataArray[randIndex1] != null) {
                            dataArray[randIndex0] = dataArray[randIndex1];//replace data in index randIndex0
                            dataArray[randIndex1] = tempData;//replace data in index randIndex1
                        }
                    }
                    count=count-1;//decrement number of swaps
                }while(count > 0);
            }
